...
if (program.cat) {
    ...
    let catagorize = function () {
        ...
        // Helper: check if a given description is unique within an array of items
        // Returns an object { unique: boolean, count: number, indexes: number[] }
        function isDescUnique(items, desc) {
            if (!Array.isArray(items)) return { unique: true, count: 0, indexes: [] }
            const indexes = []
            items.forEach((item, index) => {
                if (item && item.desc === desc) indexes.push(index)
            })
            return { unique: indexes.length <= 1, count: indexes.length, indexes }
        }

        //create a reduced list of questions for inquirer
        const reducedQuestionsList = Object.values(
            activityList
                .filter(item => !item.catComplete) //only incomplete items 
                .reduce((acc, item) => {
                    const key = item.desc
                    if (!acc[key]) {
                        ...
                        //add up field
                        acc[key].amt = currency(acc[key].amt).add(item.amt)
                    }
                    return acc
                }, {})
        )
        ...
        let qCount = 0
        reducedQuestionsList.forEach(item => {
            if (item.catComplete != true) {
                ...
                let question = {
                    ...
                    name: item.ref,
                    ref: item.ref,
                    message: `Select Catagory for: ${chalk.yellow(item.desc)}, ${item.name}, ${item.amt.toString()}, Ref: ${item.ref} `,
                    choices: choices
                }
        ...
        let bar = new ProgressBar('[:bar] :current of :total', { total: reducedQuestionsList.length, width: 20, curr: 0, incomplete: " " })
        bar.tick()
        console.log("\n")
        console.log(chalk.yellow(`There are ${chalk.red(totalUnCat1)} items left to catagorize.`))
        console.log(`Several items are the same desc. With grouping you have ${questions.length} questions to answer.`)
        ...
        function ask() {
            ...
            inquirer.prompt(questions[questionIndex])
                .then(answer => {
                    console.log(answer)
                    let catagoryId = answer[questions[questionIndex].ref]
                    let currentIndex = reducedQuestionsList.findIndex(i => i.ref == questions[questionIndex].ref)
                    let currentAmt = reducedQuestionsList[currentIndex].amt
