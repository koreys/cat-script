...
let test = function(data,x) {
    ...
    if (data.name == "SKYLER K STANLEY" && data.amexCat == "Transportation-Fuel" && +data.amt > 20){
        let index = catagories.findIndex(i => i.catName == "Skyler Gas")
        catagories[index].amount = money.add(catagories[index].amount, data.amt)
        store.autoCatAmt = money.add(store.autoCatAmt, data.amt)
        activityList[x].catComplete = true 
        return
    ...
    if (data.desc.includes("CAR WASH") || 
        ...
        data.desc.includes("ASBURY CIRCLE WASH") ||
        data.desc.includes("MB AUTO REPAIR")) {
    ...
    if (data.desc.includes("Chipotle") || 
        ...
        data.desc.includes("BURGER 2TOMS") ||
    ...
    ///////
    //////// Ofiice Supplies ////////
    if (data.desc.includes("USPS") || data.desc.includes("STAPLES") || data.desc.includes("ITUNES.COM") || data.desc.includes("HOVER") || data.desc.includes("PIRATE SHIP") ) {
    ...
    if (data.desc.includes("VULTR HOLDINGS") ||
        ...
        data.desc.includes("AAA LIFE INSURANCE") ||
        data.desc.includes("RING STANDARD PLAN") ||
    ...
    if (data.desc.includes("AMERICAN AIRLINES") ||
        data.desc.includes("UNITED AIRLINES")) {
        let index = catagories.findIndex(i => i.catName == "Travel")
        catagories[index].amount = money.add(catagories[index].amount, data.amt)
        store.autoCatAmt = money.add(store.autoCatAmt, data.amt)
        activityList[x].catComplete = true 
    ...
    if (data.desc.includes("OCEAN COUNTY LANDFIL")) {
        let index = catagories.findIndex(i => i.catName == "Waste Dump Fees")
        catagories[index].amount = money.add(catagories[index].amount, data.amt)
        store.autoCatAmt = money.add(store.autoCatAmt, data.amt)
        activityList[x].catComplete = true 
        return
    ...
    if (data.desc.includes("ABILITIES IN ACTION") || 
        ...
        data.desc.includes("THE ZEN DEN CENTER") ||
        data.desc.includes("THE ZEN DEN") ||
        data.desc.includes("CAMPI DENTAL") ||
        data.desc.includes("PURE CHIROPRACTIC") ||
        data.desc.includes("SANTO DENTAL") ||
    ...
    if (data.desc.includes("OCEAN COUNTY LANDFILL") ||
        data.desc.includes("SUBURBAN DISPOS")) {  
    ...
    if (data.desc.includes("SP * RING USA") || 
        ...
        data.desc.includes("BETTER BUSINESS BURE") ||
    ...
    if (data.desc.includes("Harbor Freight Tools") || 
        data.desc.includes("HARBOR FREIGH") ) {
    ...
    if (data.desc.includes("Agile Premium") ) {
        let index = catagories.findIndex(i => i.catName == "Insurance-Liability")
        catagories[index].amount = money.add(catagories[index].amount, data.amt)
        store.autoCatAmt = money.add(store.autoCatAmt, data.amt)
        activityList[x].catComplete = true 
        return
